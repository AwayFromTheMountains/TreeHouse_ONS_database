...
def config(filename='geoprojssd_db.ini', section='postgresql'):
    # create a parser
    parser = ConfigParser()
    # read config file
    ...
    # Returns an error if a parameter is called that is not listed in the initialization file
        ...
        raise Exception('Section {0} not found in the {1} file and cwd is {2}'.format(section, filename, os.getcwd()))
